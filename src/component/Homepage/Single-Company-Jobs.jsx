...
const SingleCompanyJobs = ({ data }) => {
  return (
    <div className="px-6 rounded-xl py-4 border-[1px] border-[#E6E7E7]">
      ...
      <h1 className="text-lg font-semibold text-[#3C3D3D] mb-[2px]">
        {data.name}
      </h1>
      <div className="flex items-center gap-2 mb-4">
        <IoLocationOutline />
        <span className="text-sm text-[#6B6D6F]">{data.address}</span>
      </div>
      <div className="flex items-center mb-4 gap-3">
        {data.tabs.map((item) => {
          return (
            <div
              key={item}
              className="rounded-[30px] border-[1px] border-[#E6E7E7] px-4 py-3"
            >
              {item}
            </div>
          );
